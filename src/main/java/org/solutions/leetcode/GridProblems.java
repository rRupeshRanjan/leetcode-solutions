...
public class GridProblems {
    ...
    public int totalNQueens(int n) {
        int[] totalNQueenCount = new int[1];
        backtrackTotalNQueens(0, n, new boolean[n], new boolean[2 * n], new boolean[2 * n], totalNQueenCount);
        return totalNQueenCount[0];
    }
     ...
     * outside the grid are considered water cells. An island in grid2 is considered a sub-island if there is an island
    ...
    public int countSubIslands(int[][] grid1, int[][] grid2) {
        ...
        for (int i = 0; i < grid1.length; i++) {
            for (int j = 0; j < grid1[0].length; j++) {
                if (grid2[i][j] == 1)
                    count += dfsCountSubIslands(grid1, grid2, i, j);
            }
        }
    ...
    private void backtrackTotalNQueens(int row, int n, boolean[] cols, boolean[] diagonals,
                                       boolean[] antiDiagonals, int[] totalNQueenCount) {
        if (row == n) {
            totalNQueenCount[0]++;
            return;
        }
        ...
        for (int col = 0; col < n; col++) {
            ...
            backtrackTotalNQueens(row + 1, n, cols, diagonals, antiDiagonals, totalNQueenCount);
    ...
    private int dfsCountSubIslands(int[][] grid1, int[][] grid2, int i, int j) {
        if (i < 0 || i == grid1.length || j < 0 || j == grid1[0].length || grid2[i][j] == 0)
            return 1;

        grid2[i][j] = 0;
        return grid1[i][j] &
                dfsCountSubIslands(grid1, grid2, i - 1, j) &
                dfsCountSubIslands(grid1, grid2, i + 1, j) &
                dfsCountSubIslands(grid1, grid2, i, j - 1) &
                dfsCountSubIslands(grid1, grid2, i, j + 1);
    }
