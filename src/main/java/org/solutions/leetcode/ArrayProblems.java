...
public class ArrayProblems {
     ...
     * <p>
     * tags:: array, interval-overlap
     */
    public int findMinArrowShots(int[][] points) {
        int arrowCount = 1;
        Arrays.sort(points, Comparator.comparingInt(a -> a[1]));

        int currArrowPos = points[0][1];
        for (int[] point : points) {
            if (point[0] > currArrowPos) {
                arrowCount++;
                currArrowPos = point[1];
            }
        }
     ...
     * between 0 and n-1. Function calls are stored in a call stack: when a function call starts, its ID is pushed onto
     * the stack, and when a function call ends, its ID is popped off the stack. The function whose ID is at the top of
     * the stack is the current function being executed. Each time a function starts or ends, we write a log with the
     ...
     * For example, if a function is called twice, one call executing for 2 time units and another call executing for 1
    ...
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        for (String log : logs) {
            Log curr = new Log(log);
            if (curr.getIsStart()) {
                stack.push(curr);
            } else {
                int currTime = (curr.getTime() - stack.pop().getTime() + 1);
                time[curr.getIndex()] += currTime;
                if (!stack.isEmpty()) {
                    time[stack.peek().getIndex()] -= currTime;
                }
            }
        }
